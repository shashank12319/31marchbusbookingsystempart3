...
public class TravelScheduleService {
	...
	public ResponseEntity<?> createSchedule(TravelScheduleDTO travelScheduleDTO) throws ParseException {
		...
		try {
	        ...
	        if (estimatedArrivalTime.isEqual(estimatedDepartureTime)) {
	            logger.warn("Estimated arrival time cannot be equal to estimated departure time: {}", travelScheduleDTO);
	            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Estimated arrival time cannot be equal to estimated departure time");
	        }

	        if (estimatedArrivalTime.isAfter(estimatedDepartureTime)) {
	            logger.warn("Estimated arrival time cannot be after estimated departure time: {}", travelScheduleDTO);
	            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Estimated arrival time cannot be after estimated departure time");
	        }
